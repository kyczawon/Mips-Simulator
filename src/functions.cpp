...
int32_t read_mem_s(uint32_t address, uint8_t (&data)[DATA_SIZE]){
...
void execute_I (uint32_t instr, uint32_t  (&data)[DATA_SIZE], int32_t (&registers)[32], uint8_t &opcode){
	...
	//filter 0x1_
		...
		switch(opcode){
			...
			// case 37: lhu(---);
			// //sb 	rt, imm(rs) 	101000
			case 40: sb(registers[src_reg] + immediate, data, registrers[dest_reg]);
			// //sh 	rt, imm(rs) 	101001
			case 41: sh(registers[src_reg] + immediate, data, registrers[dest_reg]); 
			// //sw 	rt, imm(rs) 	101011
			case 43: sw(registers[src_reg] + immediate, data, registrers[dest_reg]);
		}
	}
...
void decode_fields_R (int32_t &dest_reg, int32_t &op1, int32_t &op2, int32_t &shift_amt, int32_t &funct_code, const int32_t &instr){
