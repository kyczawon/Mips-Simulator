...
int main(int argc, char* argv[])
{
    ...
    //         int32_t result = get_simulator_output();
...

    {
        ...
        string instr, instr_name, hexaddress;
        int input1, expected_result, is_store;
        while (instructions >> instr >> instr_name >> hexaddress >> input1 >> expected_result >> is_store) {
            string binary_name = "test/temp/binary.bin";
            binary.open(binary_name.c_str());
            if (!binary.is_open()) {
                cout << "binary file could not be created" << endl;
            ...
            int address = (int)strtol(hexaddress.c_str(), NULL, 0);
            string address_binary = int_to_bin(address);
            string input1_binary = int_to_bin(input1);
            ...
            if (!is_store) { //if load, store a value and test load
                ...
                ss << instr << "0001110010" << address_binary.substr(16, 32) << endl;
            }
            if (is_store) { //if store, test store and load back
                ...
                ss << instr << "0001110000" << address_binary.substr(16, 32) << endl;
                //lw $s2 address (16lsb) v1 (address 16msb)
                ss << "1000110001110010" << address_binary.substr(16, 32) << endl;
            }
